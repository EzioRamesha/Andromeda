			...
			return e.perfectScrollbar = function(e) {
				...
				E(M).find(K).each(function(e, t) {
					var n = t,
						...
						l = E(E(n))[0].href,
						br = String(window.location),
						bl = E(E(n))[0].href;
					var lastChar = l.substr(l.length - 1);

					// to remove for compare
					r = r.replace('/Create', '');
					r = r.replace(/\/Edit\/\d{0,16}/, '');
					r = r.replace(/\/Delete\/\d{0,16}/, '');
					r = r.replace(/\/Detail\/\d{0,16}/, '');
					r = r.replace(/\/Details\/\d{0,16}/, '');
					r = r.replace(/\/EditRiDataFile\/\d{0,16}/, '');

					// to remove for compare
					l = l.replace('/Create', '');
					l = l.replace('/\/Edit\/\d{0,16}/', '');
					l = l.replace('/\/Delete\/\d{0,16}/', '');
					l = l.replace('/\/Detail\/\d{0,16}/', '');
					l = l.replace('/\/Details\/\d{0,16}/', '');
					l = l.replace('/\/EditRiDataFile\/\d{0,16}/', '');

					if (r == l || br == bl) {
						E(n).addClass(P).parents(R).add(n).each(function(e, t) {
							E(n = t).parent().addClass(k)
						});
					}

					// "#" === r.substr(r.length - 1) && (r = r.slice(0, -1)), E(E(n))[0].href.includes(r) && E(n).addClass(P).parents(R).add(n).each(function(e, t) {
					// 	E(n = t).parent().addClass(k)
					// })
				})
			}, e._addEventListeners = function() {
