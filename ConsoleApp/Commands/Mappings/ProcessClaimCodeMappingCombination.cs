...
{
    public class ProcessClaimCodeMappingCombination : Command
    {
        public ProcessClaimCodeMappingCombination()
        {
            Title = "ProcessClaimCodeMappingCombination";
            Description = "To process combination for Claim Code Mapping";
        ...
        public override void Run()
        {
            ...
            using (var db = new AppDbContext())
            {
                try
                {
                    db.Database.ExecuteSqlCommand("TRUNCATE TABLE [ClaimCodeMappingDetails]");
                    PrintMessage("Truncated [ClaimCodeMappingDetails] table");

                    int total = db.ClaimCodeMappings.Count();
                    ...
                    for (int skip = 0; skip < (total + take); skip += take)
                    {
                        ...
                        foreach (var data in db.ClaimCodeMappings.OrderBy(q => q.Id).Skip(skip).Take(take).ToList())
                        {
                            SetProcessCount();

                            var bo = ClaimCodeMappingService.FormBo(data);
                            foreach (var detailBo in ClaimCodeMappingService.CreateDetails(bo, bo.CreatedById))
                            {
                                var d = detailBo;
                                ClaimCodeMappingService.TrimMaxLength(ref d, ref list);

                                SetProcessCount("Combination");
                                db.ClaimCodeMappingDetails.Add(ClaimCodeMappingDetailService.FormEntity(detailBo));
                            }
                        }
                        db.SaveChanges();
                        ...
                        total = db.ClaimCodeMappings.Count();
                    }
                }
                catch (Exception e)
                {
