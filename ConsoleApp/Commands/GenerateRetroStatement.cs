...
{
    public class GenerateRetroStatement : Command
    {
        public RetroStatementBo RetroStatementBo { get; set; }

        public Excel RetroStatementExcel { get; set; }
        ...
        public GenerateRetroStatement()
        {
            Title = "GenerateRetroStatement";
            Description = "To generate retro statement excel file";
        }
        ...
        public void Process()
        {
            ...
            SwissReNames = new List<string> { "swissre", "swissresg" };

            string partyName = RetroStatementBo.RetroPartyBo.Party.Replace(" ", "").ToLower();
            IsSwissRe = SwissReNames.Contains(partyName);
            ...
            RetroStatementExcel.OpenTemplate();
            ...
            foreach (Column col in Column)
            {
                ...
                {
                    case TypeInvoiceNo1:
                        if (string.IsNullOrEmpty(RetroStatementBo.AccountingPeriod))
                        {
                            value = null;
                        }
                        else
                        {
                            var retroRegister = RetroRegisterService.FindByDirectRetroIdRetroPartyIdRiskQuarter(RetroStatementBo.DirectRetroId, RetroStatementBo.RetroPartyId, RetroStatementBo.AccountingPeriod);
                            if (retroRegister != null)
                                value = !string.IsNullOrEmpty(retroRegister.RetroStatementNo) ? retroRegister.RetroStatementNo : "Pending Invoice";
                            else
                                value = "No Invoice Created";
                        }
                        break;
                    case TypeInvoiceNo2:
                        if (string.IsNullOrEmpty(RetroStatementBo.AccountingPeriod2))
                        {
                            value = null;
                        }
                        else
                        {
                            var retroRegister = RetroRegisterService.FindByDirectRetroIdRetroPartyIdRiskQuarter(RetroStatementBo.DirectRetroId, RetroStatementBo.RetroPartyId, RetroStatementBo.AccountingPeriod2);
                            if (retroRegister != null)
                                value = !string.IsNullOrEmpty(retroRegister.RetroStatementNo) ? retroRegister.RetroStatementNo : "Pending Invoice";
                            else
                                value = "No Invoice Created";
                        ...
                        break;
                    case TypeInvoiceNo3:
                        if (string.IsNullOrEmpty(RetroStatementBo.AccountingPeriod3))
                        {
                            value = null;
                        }
                        else
                        {
                            var retroRegister = RetroRegisterService.FindByDirectRetroIdRetroPartyIdRiskQuarter(RetroStatementBo.DirectRetroId, RetroStatementBo.RetroPartyId, RetroStatementBo.AccountingPeriod3);
                            if (retroRegister != null)
                                value = !string.IsNullOrEmpty(retroRegister.RetroStatementNo) ? retroRegister.RetroStatementNo : "Pending Invoice";
                            else
                                value = "No Invoice Created";
                        ...
                        break;
                    default:
                        value = RetroStatementBo.GetPropertyValue(col.Property);
                        break;
                }

                RetroStatementExcel.WriteCell(col.RowIndex.Value, col.ColIndex.Value, value);
            }

            RetroStatementExcel.Save();
        }
        ...
        public void GenerateDefaultFile()
        {
            ...
            FileName = string.Format("RetroStatement_{0}", RetroStatementBo.RetroPartyBo.Party).AppendDateTimeFileName(".xlsx");
            Directory = Util.GetRetroStatementDownloadPath();
            FilePath = Path.Combine(Directory, FileName);
            RetroStatementExcel = new Excel(templateFilepath, FilePath, 1);
        }

        public void GenerateSwissReFile()
        {
            Column = GetSwissReColumns();

            var templateFilepath = Util.GetWebAppDocumentFilePath("RetroStatement_SwissRe_Export.xlsx");
            FileName = string.Format("RetroStatement_{0}", RetroStatementBo.RetroPartyBo.Party).AppendDateTimeFileName(".xlsx");
            Directory = Util.GetRetroStatementDownloadPath();
            FilePath = Path.Combine(Directory, FileName);
            RetroStatementExcel = new Excel(templateFilepath, FilePath, 1);
