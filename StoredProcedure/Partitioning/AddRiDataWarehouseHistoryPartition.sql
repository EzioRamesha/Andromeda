CREATE OR ALTER PROCEDURE [dbo].[AddRiDataWarehouseHistoryPartition](@CutOffId INT, @DatabaseName VARCHAR(MAX), @FilePath VARCHAR(MAX), @FileCount INT, @Result VARCHAR(MAX) OUTPUT)
...
BEGIN
    ...
    CREATE CLUSTERED INDEX [ClusteredIndex_on_CutOffRiDataWarehouseHistoryScheme_637722368139085366] ON [dbo].[RiDataWarehouseHistories]
    (
        [CutOffId]
    )WITH (SORT_IN_TEMPDB = OFF, DROP_EXISTING = OFF, ONLINE = OFF) ON [CutOffRiDataWarehouseHistoryScheme]([CutOffId])

    DROP INDEX [ClusteredIndex_on_CutOffRiDataWarehouseHistoryScheme_637722368139085366] ON [dbo].[RiDataWarehouseHistories]
