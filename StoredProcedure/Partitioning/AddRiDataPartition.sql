CREATE OR ALTER PROCEDURE [dbo].[AddRiDataPartition](@TreatyCodeId INT, @DatabaseName VARCHAR(MAX), @FilePath VARCHAR(MAX), @RiDataFileCount INT, @RiDataWarehouseFileCount INT, @Result VARCHAR(MAX) OUTPUT)
...
SELECT @CedantCode = c.Code FROM Cedants AS c
LEFT JOIN Treaties AS t ON
c.Id = t.CedantId
LEFT JOIN TreatyCodes AS tc ON
t.Id = tc.TreatyId
WHERE tc.Id = @TreatyCodeId;

SELECT @TreatyCode = Code FROM TreatyCodes WHERE Id = @TreatyCodeId;
...
IF NOT EXISTS (SELECT 1 FROM sys.filegroups WHERE name = @WarehouseFileGroupName)
BEGIN
...
SET @Counter=1
WHILE ( @Counter <= @RiDataWarehouseFileCount)
BEGIN
    -- RiDataWarehouse
