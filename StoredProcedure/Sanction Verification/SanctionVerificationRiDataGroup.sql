CREATE OR ALTER PROCEDURE [dbo].[SanctionVerificationRiDataGroup](@SanctionIds IdTable READONLY, @SanctionVerificationId INT)
...
DECLARE RiDataCursor CURSOR FOR
	WITH RiDataGroup AS (
		SELECT RiData.*, Cedants.Code, RANK() OVER (PARTITION BY GroupPolicyNumber, GroupPolicyName ORDER BY RiData.Id DESC, ReportPeriodYear DESC, ReportPeriodMonth DESC) RecordNo
		FROM RiData
		...
		WHERE FundsAccountingTypeCode = 'GROUP'
	) 
	SELECT 
		Id,
    ...
    FROM 
        RiDataGroup
    WHERE 
    	GroupPolicyNumber IS NOT NULL AND 
    	GroupPolicyName IS NOT NULL AND 
    	RecordNo = 1
